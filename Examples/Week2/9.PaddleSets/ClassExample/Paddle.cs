...
namespace ClassExample
{
    ...
    /// </summary>
    public abstract class Paddle : XNACS1Rectangle
    {
        ...
        abstract protected float InitialPosition();
        abstract protected float RotatedAngle(float userInput);
        abstract protected bool ShouldSetAngle(float angle);
        abstract protected bool NotAtRestPosition();
        abstract protected void UpdateRotateAngle();
    ...
    public class LeftPaddle : Paddle
    {
        ...
        public LeftPaddle(Vector2 pos) : base(pos) { }

        protected override float InitialPosition() { return kLeftRestPosition; }
        protected override float RotatedAngle(float userInput) { return kLeftRestPosition + (userInput * kAngleDisplacement); }

        protected override bool ShouldSetAngle(float angle)
        {
            return (angle >= RotateAngle);
        }

        protected override bool NotAtRestPosition()
        {
            return RotateAngle > kLeftRestPosition;
        }

        protected override void UpdateRotateAngle()
        {
    ...
    {
        protected const float kRightRestPosition = kAngleDisplacement - kMaxUpRotation;

        public RightPaddle(Vector2 pos) : base(pos) { }

        protected override float InitialPosition() { return kRightRestPosition; }
        protected override float RotatedAngle(float userInput) { return kRightRestPosition - (userInput * kAngleDisplacement); }
        ...
        protected override bool NotAtRestPosition()
        {
            return RotateAngle < kRightRestPosition;
        }

        protected override void UpdateRotateAngle()
        {
            RotateAngle += kRotateDownSpeed;
            if (RotateAngle > kRightRestPosition)
            {
                RotateAngle = kRightRestPosition;
                mCurrentPaddleState = PaddleState.PaddleAtRest;
            }
        }
    }
